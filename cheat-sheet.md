...
| Filtrado                     | `df[condición]` <br> `subset(df, condición)` | El argumento `condición` es una expresión lógica indicando lo que se quiere tomar. Ejemplo: `df[df$sexo == "Hombre"]`. Puedes combinarlos de varias formas. Por ejemplo, con rangos, como `df[1:2, c("edad", "estudios")` <br> La función `subset` también sirve para otro tipo de estructuras |
